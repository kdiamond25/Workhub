              ...
              <div style={{background:'#FAEEDA',border:'1px solid #EF9F27',padding:'14px 20px'}}>
                ...
                {lowConf.map(e=>(
                  <div key={e.id} style={{display:'flex',alignItems:'center',gap:12,padding:'7px 0',borderTop:'1px solid rgba(186,117,23,0.2)'}}>
                    ...
                    <button onClick={()=>{setChatInput(`Email "${e.subject}" was classified as ${e.project} — is that right?`);setPage('agent')}} style={{fontSize:11,padding:'4px 12px',borderRadius:2,flexShrink:0}}>Ask agent</button>
                  </div>
                ))}
              </div>
            )}
                  ...
                  return(
                    <div key={m.id} style={{padding:'12px 20px',borderBottom:'1px solid #e8e8e4',display:'flex',gap:14,alignItems:'center'}}>
                      ...
                      <div style={{flex:1}}>
                        ...
                        <div style={{display:'flex',gap:8}}><span style={{fontSize:12,color:'#888'}}>{m.time}</span>{tag(m.project,m.project)}</div>
                      </div>
                    </div>
                  )
                })}
              ...
              {tasks.map(t=>(
                <div key={t.id} style={{padding:'14px 20px',borderBottom:'1px solid #e8e8e4',display:'flex',gap:14,alignItems:'flex-start'}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{display:'flex',gap:8}}>{t.project&&tag(t.project,t.project)}{t.due&&<span style={{fontSize:12,color:'#888'}}>Due {t.due}</span>}</div>
                  </div>
                </div>
              ))}
            </div>
                ...
                return(
                  <div key={m.id} style={{padding:'16px 20px',borderBottom:'1px solid #e8e8e4',display:'flex',gap:16,alignItems:'center',borderLeft:`4px solid ${c.a}`}}>
                    ...
                    <div>
                      ...
                      <div style={{display:'flex',gap:10}}><span style={{fontSize:13,color:'#888'}}>{m.time}</span>{tag(m.project,m.project)}</div>
                    </div>
                  </div>
                )
              })}
        ...
        {page==='agent'&&(
          <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
            ...
            <div style={{padding:'8px 20px',background:'#f4f4f0',borderTop:'1px solid #e8e8e4',display:'flex',gap:6,flexWrap:'wrap'}}>
              {['Triage all emails','Show emails needing reply','What tasks are open?','Draft reply to most recent high priority email','Fix any low-confidence classifications'].map(s=>(
